...
export function Footer() {
  return <footer className="footer"><div className="container footer-grid">
    ...
    <div className="footer-contact"><h3>Contact</h3><a href={company.phoneHref} className="contact-line"><Phone size={16}/><span>{company.phone}</span></a><a href={company.emailHref} className="contact-line"><Mail size={16}/><span>{company.email}</span></a><a href={company.addressHref} target="_blank" rel="noreferrer" className="contact-line"><MapPin size={16}/><span>{company.address}</span></a><p className="contact-line"><Clock3 size={16}/><span>{company.businessHours}</span></p><Link href="/contact" className="footer-link">Request a quote →</Link></div>
